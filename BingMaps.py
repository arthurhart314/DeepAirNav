...
def calculateLeftCorner(latitude, longitude, side_length):
    ...
    r_earth = 6378.0
    left_corner_latitude = latitude - (side_length * 0.5) / (r_earth * 1000.0) * (180.0 / math.pi)
    left_corner_longitude = longitude - (side_length * 0.5) / (r_earth * 1000.0) * (180.0 / math.pi) / math.cos(latitude * math.pi / 180.0)
...
def calculateRightCorner(latitude, longitude, side_length):
    ...
    """
    r_earth = 6378.0
    right_corner_latitude = latitude + (side_length * 0.5) / (r_earth * 1000.0) * (180.0 / math.pi)
    right_corner_longitude = longitude + (side_length * 0.5) / (r_earth * 1000.0) * (180.0 / math.pi) / math.cos(latitude * math.pi / 180.0)

    print(right_corner_latitude, right_corner_longitude)
    return right_corner_latitude, right_corner_longitude


def calculateCorner(lat, lon, side_length, corner):
    r_earth = 6378.0
    corner_lat = lat + corner[0] * (side_length * 0.5) / (r_earth * 1000.0) * (180.0 / math.pi)
    corner_lon = lon + corner[1] * (side_length * 0.5) / (r_earth * 1000.0) * (180.0 / math.pi) / math.cos(lat * math.pi / 180.0)
...
def downloadAerialImagery(upper_left_tile, lower_right_tile, filename, tmp_folder, level):
    tiles = 0
    tilename = "tile"
    all_images = []
    for y in range(upper_left_tile[1], lower_right_tile[1] + 1):
        ...
        for x in range(upper_left_tile[0], lower_right_tile[0] + 1):
            ...
            cv2.imwrite(tmp_folder + '/' + tilename, image)
            tilename = "tile"
...
def load(lat, lon, width, filename, tmp_dir):
    start_time = time.time()

    if os.path.exists(tmp_dir):
        os.system('rm -r ' + tmp_dir)

    p1_latitude, p1_longitude = calculateCorner(lat, lon, width, [ 1,  1])
    p2_latitude, p2_longitude = calculateCorner(lat, lon, width, [-1, -1])
    ...
    image = downloadAerialImagery(t1, t2, filename, tmp_dir, level)
